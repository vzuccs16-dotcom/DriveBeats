...
window.addEventListener('DOMContentLoaded', () => {
  ...
  // If CLIENT_ID is hardcoded, hide the input step and show a status badge
  const hardcoded = CLIENT_ID && CLIENT_ID !== 'PASTE_YOUR_CLIENT_ID_HERE';
  if (hardcoded) {
    const step = document.querySelector('.setup-steps');
    step.innerHTML = `<div class="hardcoded-badge">✅ Client ID loaded from <code>app.js</code></div>`;
  } else {
...
async function connectDrive() {
  // Use hardcoded CLIENT_ID if filled in, otherwise fall back to the input field
  const hardcoded = CLIENT_ID && CLIENT_ID !== 'PASTE_YOUR_CLIENT_ID_HERE';
  const clientId = hardcoded
    ? CLIENT_ID
    : document.getElementById('client-id-input').value.trim();
